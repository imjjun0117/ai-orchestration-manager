-- Phase 18: allow existing worker principals to read every task column used by
-- the claim-bound memory context builder. Memory tables remain inaccessible.
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'GRANT SELECT (title, memory_project_key) ON TABLE tasks TO %I',
